...
def crea_grafici(dati):
    ...
    for numero in tutti_numeri:
        if numero in frequenze:
            ...
            frequenze[numero] = 1
    ax1.bar(frequenze.keys(), frequenze.values(), color='skyblue')
    ax1.set_title('Frequenza dei Numeri', fontsize=16, fontweight='bold')
    ax1.set_xlabel('Numero', fontsize=14)
    ...
    for _, sequenza in dati:
        ax2.plot(sequenza, label='Sequenza', linestyle='-', linewidth=2)
    ax2.set_title('Andamento dei Numeri nel Tempo', fontsize=16, fontweight='bold')
    ax2.set_xlabel('Posizione', fontsize=14)
    ...
    ax3.hist(tutti_numeri, bins=range(min(tutti_numeri), max(tutti_numeri)+2), align='left', rwidth=0.8, color='lightgreen')
    ax3.set_title('Distribuzione dei Numeri', fontsize=16, fontweight='bold')
    ax3.set_xlabel('Numero', fontsize=14)
    ax3.set_ylabel('Frequenza', fontsize=14)
    ...
    ax4.plot(sorted_data, cdf, color='orange', linestyle='--', linewidth=2)
    ax4.set_title('Probabilità Cumulativa', fontsize=16, fontweight='bold')
    ax4.set_xlabel('Numero', fontsize=14)
    ax4.set_ylabel('Probabilità Cumulativa', fontsize=14)
    ...
    def __init__(self):
        ...
        for anno in sorted(anni):
            ...
            for i, sequenza in enumerate(sequenze_anno):
                sub_ax.plot(sequenza, label=f'Sequenza {i+1}', linestyle='--', linewidth=2, color=plt.cm.tab20(i))
            sub_ax.set_title(f'Andamento dei Numeri nel Tempo ({anno})', fontsize=16, fontweight='bold')
            sub_ax.set_xlabel('Posizione', fontsize=14)
            sub_ax.set_ylabel('Valore', fontsize=14)
            sub_ax.legend(loc='upper right', fontsize=12, bbox_to_anchor=(1.05, 1))
